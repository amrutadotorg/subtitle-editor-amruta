...
export default forwardRef(function WaveformVisualizer(
  ...
  const plugins = useMemo(
    () => [timelinePlugin, hoverPlugin, regionPlugin],
    [timelinePlugin, hoverPlugin, regionPlugin],
  );
  ...
  const { wavesurfer } = useWavesurfer({
    ...
    cursorColor,
    url: mediaUrl,
    peaks: extractedPeaks, // Use extracted peaks if generated
    duration: extractedDuration, // Prevent wavesurfer from decoding the file
    minPxPerSec: 100,
    fillParent: true,
    autoCenter: true,
    normalize: true,
  ...
  // Watchdog: fallback to ensure spinner doesn't hang forever
  useEffect(() => {
    if (!isLoading) return;
  ...
  // Notify user if waveform is disabled
  // Removed because waveform is now decoded in chunks
  ...
  return (
    <div className="relative w-full h-full border-black">
      ...
      {(isLoading || isExtracting) && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/50 dark:bg-black/40 z-20">
          <IconLoader2 className="text-3xl text-black dark:text-white animate-spin mb-2" />
          {isExtracting && (
            <div className="text-sm font-medium text-black dark:text-white bg-white/80 dark:bg-black/80 px-3 py-1 rounded-md">
              Generating waveform: {extractionProgress}%
            </div>
          )}
        </div>
      )}
